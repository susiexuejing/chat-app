...
const EF189_SCOPE = 'ef-189-pr-73-a1b7378-fixed-head';
...
const EF211_AUTHORITY = '7134a8c0a5583c08074575146687c4ee647ec99a';
...
const scopeObject = {
  ...
  approvedProfiles: [
    ...
    },
    {
      ...
      targetBranch: 'dev',
      sourceRepository: 'susiexuejing/chat-app', sourceBranch: 'cell2/ef107-reentry-edb772d7',
  ...
  return (root, args) => {
    ...
    if (operation === 'rev-list') {
      ...
      if (Array.isArray(candidateAncestry)) {
        ...
        if (ancestryIndex === 0) return object;
      }
...
test('EF-111 R2 admits only PR 101 at the fixed current Base with the approved patch and three paths', async t => {
  ...
  for (const entry of cases) {
    ...
    if (entry.pass) {
      ...
      assert.equal(manifest.scopeId, EF111_R2_EF177_SCOPE);
      assert.deepEqual(manifest.targetedRegressionIds, ['chat-ui-jest-path']);
      assert.equal(manifest.targetedTestPath, null);
...
test('retired EF-161 scope is explicitly rejected by Base-owned authority', async t => {
  const { root, file } = await eventFixture({
    number: 93,
    head: '12048a16386edf34e6527341ffda64181d98e4ce',
    base: EF194_EF107_AUTHORITY,
    body: `Review-Scope: ${RETIRED_EF161_SCOPE}`,
    headRef: 'cell3/ef-161-web-same-origin-backend',
    headRepository: 'susiexuejing/chat-app',
  });
  t.after(() => rm(root, { recursive: true, force: true }));
  await assert.rejects(
    createReviewManifest(
      { GITHUB_EVENT_NAME: 'pull_request', GITHUB_EVENT_PATH: file },
      optionsFor(file, { mode: 'dual', authorityRoot: '/fixed/authority', candidateRoot: '/fixed/candidate' }, gitFixture({
        authority: EF194_EF107_AUTHORITY,
        head: '12048a16386edf34e6527341ffda64181d98e4ce',
        candidateParent: '2284f0479316e3eb5a29b86854c82792aaa5a1c5',
        mergeBase: '2284f0479316e3eb5a29b86854c82792aaa5a1c5',
        changed: ['client/screens/chat/__tests__/ef75-ownership-production-path.test.tsx', 'client/screens/chat/stores/sessionStore.ts'],
      })),
    ),
...
test('EF-194 admits only the fixed EF-107 re-entry candidate after the exact seven-path governance closure', async t => {
  const layout = { mode: 'dual', authorityRoot: '/fixed/authority', candidateRoot: '/fixed/candidate' };
  const cases = [
    { pass: true },
    ...
    { head: HEAD, error: /candidate SHA/ },
    { parent: HEAD, error: /direct parent/ },
    { mergeBase: HEAD, error: /lineage|merge-base/ },
    { ancestry: EF194_EF107_ANCESTRY_SHAS.slice(1), error: /direct parent|ancestry/ },
    { ancestry: [...EF194_EF107_ANCESTRY_SHAS].reverse(), error: /direct parent|ancestry/ },
    { ancestry: ['f'.repeat(40), EF194_EF107_HEAD], error: /direct parent|ancestry/ },
    { patch: 'f'.repeat(40), error: /patch ID/ },
    { patch: '3f083ee7b5c79d5cecc41f0aa036f05f52fabcf0', error: /patch ID/ },
    { headRef: 'other-branch', error: /source identity/ },
    ...
    { baseAdvancePaths: [...EF194_EF107_BASE_ADVANCE_PATHS.slice(0, 6), EF194_EF107_PATHS[0]], error: /non-governance or overlapping paths/ },
  ...
  for (const entry of cases) {
    ...
    const ancestry = entry.ancestry ?? [parent, head];
    const { root, file } = await eventFixture({
      head, base, body: `Review-Scope: ${EF194_EF107_SCOPE}`,
      headRef: entry.headRef ?? 'cell2/ef107-reentry-edb772d7',
    ...
    const options = optionsFor(file, layout, gitFixture({
      ...
      mergeBase: entry.mergeBase ?? EF194_EF107_ORIGINAL_BASE,
...
test('EF-189 PR 73 admits only its fixed head, merge-base, exact paths, and one synthetic race test', async t => {
  const { root, file } = await eventFixture({
    number: 73, head: EF189_HEAD, body: `Review-Scope: ${EF189_SCOPE}`,
  });
  t.after(() => rm(root, { recursive: true, force: true }));
  const manifest = await createReviewManifest(
    { GITHUB_EVENT_NAME: 'pull_request', GITHUB_EVENT_PATH: file },
    optionsFor(file, { mode: 'dual', authorityRoot: '/fixed/authority', candidateRoot: '/fixed/candidate' },
      gitFixture({ head: EF189_HEAD, changed: EF189_PATHS, mergeBase: EF189_MERGE_BASE })),
  );
  assert.equal(manifest.scopeId, EF189_SCOPE);
  assert.deepEqual(manifest.targetedRegressionIds, ['chat-ui-jest-path']);
  assert.equal(manifest.targetedTestPath, EF189_TARGETED_TEST_PATH);
  assert.deepEqual(manifest.changedPaths, EF189_PATHS);
  assert.deepEqual(manifest.structuralProof, {
    kind: 'exact-fixed-head-targeted-test', approvedPaths: EF189_PATHS,
    approvedHeadSha: EF189_HEAD, approvedMergeBaseSha: EF189_MERGE_BASE,
  });
});

test('EF-189 PR 73 rejects changed identity, unknown target, and every missing or extra path', async t => {
  const cases = [
    { number: 74, head: EF189_HEAD, mergeBase: EF189_MERGE_BASE, changed: EF189_PATHS, error: /profile does not match/ },
    { number: 73, head: HEAD, mergeBase: EF189_MERGE_BASE, changed: EF189_PATHS, error: /head SHA is not approved/ },
    { number: 73, head: EF189_HEAD, mergeBase: MERGE_BASE, changed: EF189_PATHS, error: /merge-base SHA is not approved/ },
    { number: 73, head: EF189_HEAD, mergeBase: EF189_MERGE_BASE, changed: EF189_PATHS.slice(0, -1), error: /exact approved path set/ },
    { number: 73, head: EF189_HEAD, mergeBase: EF189_MERGE_BASE, changed: [...EF189_PATHS, 'client/screens/chat/api/cozeApi.ts'], error: /exact approved path set/ },
  ];
  for (const entry of cases) {
    const { root, file } = await eventFixture({
      number: entry.number, head: entry.head, body: `Review-Scope: ${EF189_SCOPE}`,
    });
    t.after(() => rm(root, { recursive: true, force: true }));
    await assert.rejects(createReviewManifest(
      { GITHUB_EVENT_NAME: 'pull_request', GITHUB_EVENT_PATH: file },
  ...
  const malformed = {
    ...scopeObject,
    approvedProfiles: scopeObject.approvedProfiles.map(profile => profile.id === EF189_SCOPE
