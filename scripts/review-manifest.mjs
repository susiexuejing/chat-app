...
const EF211_AUTHORITY_SHA = '551948abd520177d5d8dd1c1d3d29beb3df51e8a';
...
function verifyFixedCandidateProfileWithoutPrNumber({ profile, baseSha, headSha, mergeBaseSha, changed, candidateRoot, git, event }) {
  ...
  if (mergeBaseSha !== profile.candidateParentSha || baseSha !== profile.candidateParentSha) {
    fail('fixed candidate profile base identity is not approved');
  }
...
export async function createReviewManifest(env = process.env, options = {}) {
  ...
  if (eventName === 'pull_request') {
    ...
    const expectedAuthoritySha = requestedScopeId === EF210_SCOPE_ID ? EF210_AUTHORITY_SHA
      : requestedScopeId === EF211_PROFILE_ID ? EF211_AUTHORITY_SHA : baseSha;
    ...
    if (requestedScopeId !== null) {
      ...
      if (scope.lowRiskProfiles.has(requestedScopeId)) {
        ...
        structuralProof = verifyProfile({
          profile, baseSha, headSha, mergeBaseSha, changed, candidateRoot: layout.candidateRoot, git, event,
        });
