...
export function detectUserState(message: string): UserState {
  ...
  if (
    /怕被丢下|不理我|是不是[我]说错|是不是我不好|他是不是[讨厌烦嫌弃不理]|是不是不喜欢[我了]|嫌我|抛弃|不回了|没回消息|不回(消息|信息|微信)|没回我|已读不回|冷落[我了]？|太粘人|疏远|冷淡[了我]？|不搭理[我]|躲着我|不爱[我了]|不要我了|被丢下/.test(m)
  ) {
  ...
  // ── 3. 愤怒/生气 ──
  if (
    /生气|愤怒|火大|气死|发火|发[^。.!?]{0,10}脾[气]?|暴躁|烦死了|受不了|忍不了|爆炸|很气|委屈|凭什么|为什么总是我|每次都这样/.test(m)
  ) {
    return 'anger';
  }
  ...
  // ── 5. 自我怀疑（增强） ──
  if (
    /不够好|我很差|很差|自责|内耗|讨厌自己|看不起|没用|失败|不够努力|我真差|我不行|我做不到|自卑|是不是我错了|是不是我做错了|我做错什么|我[又]搞砸了|搞砸|总是我[的]错|背锅|都是我不好|我太差|都是我的问题|我好差|我不好|都是我[的]错|什么都做不好/.test(m)
  ) {
    return 'self_doubt';
  }
