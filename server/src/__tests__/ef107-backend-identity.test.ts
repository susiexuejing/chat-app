...
describe('EF-107 backend identity and ownership contract', () => {
  ...
  it('rejects malformed and forged/mismatched identity context', () => {
    ...
    expect(readBackendIdentity(request({ 'X-EmotionFlow-User-Id': otherUser }), { bodyUserId: owner }))
      .toEqual({ failure: 'mismatched' });
    expect(readBackendIdentity(request({ 'X-EmotionFlow-User-Id': owner, 'X-EmotionFlow-Conversation-Id': 'forged' }), {
      requireConversationHeader: true,
    })).toEqual({ failure: 'malformed' });
  });

  it('requires stream conversation context and preserves exact owner binding', () => {
    expect(readBackendIdentity(request({
      'X-EmotionFlow-User-Id': owner,
      'X-EmotionFlow-Conversation-Id': conversation,
    }), { requireConversationHeader: true })).toEqual({
      identity: { userId: owner, conversationId: conversation },
    });
    expect(readBackendIdentity(request({ 'X-EmotionFlow-User-Id': owner }), {
      requireConversationHeader: true,
    })).toEqual({ failure: 'missing' });
