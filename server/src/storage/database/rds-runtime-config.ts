/**
 * EF-235 identity database runtime-only configuration boundary.
 *
...
export function parseRdsRuntimeConfig(env: NodeJS.ProcessEnv): RdsRuntimeConfigResult {
  const host = exact(env.EF_IDENTITY_RDS_HOST, SAFE_HOST);
  const database = exact(env.EF_IDENTITY_RDS_DATABASE, SAFE_IDENTIFIER);
  const user = exact(env.EF_IDENTITY_RDS_USER, SAFE_IDENTIFIER);
  const password = env.EF_IDENTITY_RDS_PASSWORD;
  const sslCa = env.EF_IDENTITY_RDS_SSL_CA;
  const portText = env.EF_IDENTITY_RDS_PORT;
  const port = portText && /^[0-9]{1,5}$/.test(portText) ? Number(portText) : NaN;

  if (!host || !database || !user || !password || !sslCa?.trim()
    || !Number.isInteger(port) || port < 1 || port > 65535
    || isBootstrapAdministrator(user)) {
    return { ok: false };
  }

  return { ok: true, config: { host, port, database, user, password, sslCa } };
}
