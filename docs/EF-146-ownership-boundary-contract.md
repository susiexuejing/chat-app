...
| --- | --- | --- | --- | --- | --- |
| Create conversation | Verified server principal | body/header `userId`, role ID, client conversation ID | Derive the owner only from `principal.userId`. If any supplied owner/user ID is present and differs from that principal, reject before repository/provider work. A matching or absent client value is never used as persistence authority. Validate role independently. | `401 invalid_identity_context` for missing/invalid principal **or any supplied-owner mismatch**; `400` only for non-identity validation. Mismatch performs zero repository/provider calls and zero writes. | Successful-create test proves the stored owner is the verified principal. Forged/mismatched B body/header test proves `401`, no repository/provider call, and no write. |
...
| --- | --- |
| Forged owner body/header | A valid-looking B UUID in a body/header never becomes the principal. When it differs from the verified principal, return `401 invalid_identity_context` before any repository/provider call and perform zero write. A successful create persists only the verified principal owner. |
